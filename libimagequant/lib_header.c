/*--------------------------------------------------------------------------*/
/* Resident header written for imagequant.library                           */
/*--------------------------------------------------------------------------*/
...
const UBYTE lib_id[]    = "$VER: " LIBHEADNAMESTR ".library " VERS " (" COMPDATE ") AROS version by magorium\n";
...
__saveds APTR LibExpungeInternal(LIBBASETYPE *Base reg(a6))
{
  ...
  FreeMem
  (
    (APTR)Base - Base->imagequant_Lib.lib_NegSize, 
    (LONG)Base->imagequant_Lib.lib_PosSize + (LONG)Base->imagequant_Lib.lib_NegSize
  );
